...
public class FruitManager_Pixel : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 1; i < fruitPosition.Length; i++)
        {
            ...
            if (randomFruits)
            {
                ...
                newFruit.GetComponent<Fruit_Item_Pixel>().FruitSteup(fruitIndex);
            }
            else
            {
                newFruit.GetComponent<Fruit_Item_Pixel>().FruitSteup(fruitIndex);
            ...
            int levelNumber = GameManager_Pixel.instance.levelNumber;
